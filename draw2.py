...
class Draw(object):
    ...
    def draw(self):
        ...
        for src_parsed_step, trg_parsed_step, align_parsed_step in zip(src_parsed, trg_parsed, align_parsed):
            ...
            # pdb.set_trace()
            activation_map = np.asarray(align_parsed_step).reshape(len(trg_parsed_step), len(src_parsed_step)).transpose()
            # add image
            ...
            ax.set_xticks(range(len(trg_parsed_step)))

            ax.set_xticklabels(trg_parsed_step_new, fontproperties=myfont, rotation=45)

            ax.set_yticks(range(len(src_parsed_step)))

            ax.set_yticklabels(src_parsed_step_new, fontproperties=myfont)
            # set x&y axis props
