...
class Draw(object):
    ...
    def force_split(self, data):
        data_temp = data.strip().split('/')
        data_used = data_temp[0]
        data_final = data_used.strip().split(':')
        return data_final

    def get_align(self, src, trg, data):
        #pdb.set_trace()
        align_final = []
        for src_step, trg_step, data_step in zip(src, trg, data):
            src_len = len(src_step)
            trg_len = len(trg_step)
            values = numpy.zeros((trg_len, src_len))
            for data_now in data_step:
                values[int(data_now[1])-1][int(data_now[0])-1] = 1
            align_final.append(values)
        return align_final
    ...
    def split_force_align_data(self, data):
        splited_data = []
        for now_data in data:
            force_data=[]
            splited_now_data = now_data.strip().split(' ')
            for step in splited_now_data:
                force_splited_data = self.force_split(step)
                force_data.append(force_splited_data)
            splited_data.append(force_data)
        return splited_data
    ...
    def parse_data(self):
        ...
        align_splited = self.split_force_align_data(align_txt)

        align_final = self.get_align(src_splited, trg_splited, align_splited)

        src_obj.close()
        trg_obj.close()
        align_obj.close()
        return src_splited, trg_splited, align_final
    ...
    def draw(self):
        ...
        for src_parsed_step, trg_parsed_step, align_parsed_step in zip(src_parsed, trg_parsed, align_parsed):
            ...
            activation_map = align_parsed_step
            # add image
